...
-(void) insertData{
    ...
    NSString *insertSql1= [NSString stringWithFormat:
						   @"INSERT INTO '%@' ('%@', '%@', '%@', '%@', '%@', '%@', '%@') VALUES ('%@', '%@', '%@', '%@', '%@', '%@', '%@')",
						   TABLENAME, PID, NAME, DESCRIPTION, REGULAR_PRICE, SALE_PRICE, COLORS, STORES, [dic1 objectForKey:@"id"], [dic1 objectForKey:@"name"], [dic1 objectForKey:@"description"], @"1.1", @"1.2", strColors, strStores];
...
{
    DetailViewController *dvc = [[DetailViewController alloc] initWithNibName:@"DetailViewController" bundle:nil];
    dvc.jsonDic = [_dataArray objectAtIndex:indexPath.row];
    dvc.deleteProduct = ^{
        NSLog(@"deleteProduct is delete");
        [self deleteData:[[_dataArray objectAtIndex:indexPath.row] objectForKey:@"id"]];
    };
    [self presentViewController:dvc animated:YES completion:^{
        // do some thing
    }];
